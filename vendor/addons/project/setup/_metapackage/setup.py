import setuptools

with open('VERSION.txt', 'r') as f:
    version = f.read().strip()

setuptools.setup(
    name="odoo-addons-oca-project",
    description="Meta package for oca-project Odoo addons",
    version=version,
    install_requires=[
        'odoo-addon-project_administrator_restricted_visibility>=16.0dev,<16.1dev',
        'odoo-addon-project_department>=16.0dev,<16.1dev',
        'odoo-addon-project_duplicate_subtask>=16.0dev,<16.1dev',
        'odoo-addon-project_hr>=16.0dev,<16.1dev',
        'odoo-addon-project_key>=16.0dev,<16.1dev',
        'odoo-addon-project_list>=16.0dev,<16.1dev',
        'odoo-addon-project_merge>=16.0dev,<16.1dev',
        'odoo-addon-project_milestone_tree>=16.0dev,<16.1dev',
        'odoo-addon-project_parent>=16.0dev,<16.1dev',
        'odoo-addon-project_parent_task_filter>=16.0dev,<16.1dev',
        'odoo-addon-project_pivot>=16.0dev,<16.1dev',
        'odoo-addon-project_purchase_link>=16.0dev,<16.1dev',
        'odoo-addon-project_role>=16.0dev,<16.1dev',
        'odoo-addon-project_sequence>=16.0dev,<16.1dev',
        'odoo-addon-project_stage_extra_info>=16.0dev,<16.1dev',
        'odoo-addon-project_stage_last_update_date>=16.0dev,<16.1dev',
        'odoo-addon-project_stock>=16.0dev,<16.1dev',
        'odoo-addon-project_stock_analytic_tag>=16.0dev,<16.1dev',
        'odoo-addon-project_stock_product_set>=16.0dev,<16.1dev',
        'odoo-addon-project_tag_hierarchy>=16.0dev,<16.1dev',
        'odoo-addon-project_tag_multicompany>=16.0dev,<16.1dev',
        'odoo-addon-project_tag_security>=16.0dev,<16.1dev',
        'odoo-addon-project_task_add_very_high>=16.0dev,<16.1dev',
        'odoo-addon-project_task_code>=16.0dev,<16.1dev',
        'odoo-addon-project_task_default_stage>=16.0dev,<16.1dev',
        'odoo-addon-project_task_description_template>=16.0dev,<16.1dev',
        'odoo-addon-project_task_link>=16.0dev,<16.1dev',
        'odoo-addon-project_task_material>=16.0dev,<16.1dev',
        'odoo-addon-project_task_name_with_id>=16.0dev,<16.1dev',
        'odoo-addon-project_task_note>=16.0dev,<16.1dev',
        'odoo-addon-project_task_personal_stage_auto_fold>=16.0dev,<16.1dev',
        'odoo-addon-project_task_project_required>=16.0dev,<16.1dev',
        'odoo-addon-project_task_pull_request>=16.0dev,<16.1dev',
        'odoo-addon-project_task_pull_request_state>=16.0dev,<16.1dev',
        'odoo-addon-project_task_stage_mgmt>=16.0dev,<16.1dev',
        'odoo-addon-project_task_stage_state>=16.0dev,<16.1dev',
        'odoo-addon-project_template>=16.0dev,<16.1dev',
        'odoo-addon-project_timeline>=16.0dev,<16.1dev',
        'odoo-addon-project_timeline_hr_timesheet>=16.0dev,<16.1dev',
        'odoo-addon-project_timesheet_time_control>=16.0dev,<16.1dev',
        'odoo-addon-project_type>=16.0dev,<16.1dev',
        'odoo-addon-project_update_visible>=16.0dev,<16.1dev',
        'odoo-addon-sale_project_copy_tasks>=16.0dev,<16.1dev',
        'odoo-addon-task_project_status>=16.0dev,<16.1dev',
    ],
    classifiers=[
        'Programming Language :: Python',
        'Framework :: Odoo',
        'Framework :: Odoo :: 16.0',
    ]
)
